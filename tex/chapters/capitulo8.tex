...
\begin{figure*}[h!]
    ...
    \begin{tikzpicture}
        ...
        \draw[
            decoration={
                markings, mark=at position 0.51 with {\arrow[>={Stealth[scale=1.25]}]{>}}
            },
            postaction={decorate}
        ] (O) circle (\radius);
        \draw[
...
\begin{example}
    Cierta familia está compuesta por una madre, un padre, una hija y dos hijos. Los miembros de la familia tienen influencia o poder entre ellos de la siguiente manera: la madre puede influir en la hija y el hijo mayor; el padre puede influir en los dos hijos; la hija puede influir en el padre; el hijo mayor puede influir en el hijo menor; y el hijo menor puede influir en la madre. Podemos modelar este patrón de influencia familiar con un grafo dirigido cuyos vértices son los cinco miembros de la familia. Si el miembro de la familia $A$ influye en el miembro de la familia $B$, escribimos $A \rightarrow B$. La Figura 10.5.4 es el grafo dirigido resultante, donde hemos utilizado designaciones obvias para los cinco miembros de la familia. La matriz de vértices de este grafo dirigido es
\end{example}
